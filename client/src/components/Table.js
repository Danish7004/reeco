          ...
          {!loading && data.length !== 0
            ? data.map((el, i) => (
                <tr style={{ borderBottom: "1px solid #cecece" }} key={i}>
                  <td style={{ padding: "15px 0px" , display:'flex',  alignItems:'center'}}><span><img src={pic1} alt="" width="30px" /></span>{el.name}</td>
                  <td>{el.brand}</td>
                  <td>{el.price}</td>
                  <td>{el.qty}</td>
                  ...
                  <td>
                    ...
                    >
                     ...
                     <div> {
                      el.status === "Missing" &&
                      <Status style={{backgroundColor: 'tomato'}}>{el.status}</Status>
                    }
                      {
                      el.status === "Missing-urgent" &&
                      <Status style={{backgroundColor: 'red'}}>{el.status}</Status>
                    }
                      {
                      el.status === "Approved" &&
                      <Status style={{backgroundColor: '#4de14d'}}>{el.status}</Status>
                    }</div>
                    : <p></p>}
